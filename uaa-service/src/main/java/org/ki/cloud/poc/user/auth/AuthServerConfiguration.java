...
public class AuthServerConfiguration extends AuthorizationServerConfigurerAdapter {
  ...
  public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
    clients.inMemory().withClient("web").secret("secret").authorities("ROLE_WEB")
        .scopes("read", "write", "trust").authorizedGrantTypes("client_credentials",
...
class Oauth2Config {
  ...
  public TokenStore tokenStore(ApprovalStore approvalStore) {
    JwtTokenStore jwtTokenStore = new JwtTokenStore(jwtAccessTokenConverter()) {
      ...
      public OAuth2RefreshToken readRefreshToken(String tokenValue) {
        ...
        OAuth2RefreshToken readRefreshToken = super.readRefreshToken(
            Optional.ofNullable(jwtCache.get(tokenValue))
                .orElseThrow(() -> new InvalidTokenException(
                    String.format("Invalid refresh token: %s", tokenValue))));

        return new DefaultExpiringOAuth2RefreshToken(tokenValue,
            ((DefaultExpiringOAuth2RefreshToken) readRefreshToken).getExpiration());
      }
      ...
      public OAuth2Authentication readAuthenticationForRefreshToken(
        ...
        return super.readAuthenticationForRefreshToken(
            Optional.ofNullable(jwtCache.get(token.getValue()))
                .map(jwt -> new DefaultExpiringOAuth2RefreshToken(jwt,
                    ((DefaultExpiringOAuth2RefreshToken) token).getExpiration()))
                .orElseThrow(() -> new InvalidTokenException(
                    String.format("Invalid refresh token: %s", token.getValue()))));

      }

      @Override
      public void removeRefreshToken(OAuth2RefreshToken token) {
        super.removeRefreshToken(Optional.ofNullable(jwtCache.get(token.getValue()))
            .map(jwt -> new DefaultExpiringOAuth2RefreshToken(jwt,
                ((DefaultExpiringOAuth2RefreshToken) token).getExpiration()))
            .orElseThrow(() -> new InvalidTokenException(
                String.format("Invalid refresh token: %s", token.getValue()))));
      ...
      public void storeRefreshToken(OAuth2RefreshToken refreshToken,
        ...
        if (null != approvalStore) {
          String tokenValue = Optional.of(jwtCache.get(refreshToken.getValue()))
              .orElseThrow(() -> new InvalidTokenException(
                  String.format("Invalid refresh token: %s", refreshToken.getValue())));
          ...
          if (user != null) {
            ...
            for (String scope : auth.getOAuth2Request().getScope()) {
              approvals.add(new Approval(user.getName(), clientId, scope, date,
  ...
  protected JwtAccessTokenConverter jwtAccessTokenConverter() {
      ...
      public OAuth2AccessToken enhance(OAuth2AccessToken accessToken,
        ...
        if (authentication.getPrincipal() instanceof AuthUser) {
          ...
          additionalInfo.put("refresh_token_expires_in",
              (((ExpiringOAuth2RefreshToken) preEnhancedToken.getRefreshToken())
                  .getExpiration().getTime() - System.currentTimeMillis()) / 1000);
        }

        preEnhancedToken.setAdditionalInformation(additionalInfo);
        OAuth2AccessToken enhancedToken = super.enhance(preEnhancedToken, authentication);
    ...
    final KeyPair keyPair = new KeyStoreKeyFactory(new ClassPathResource("spring.jks"),
        "spring".toCharArray()).getKeyPair("spring", "spring".toCharArray());
